...
import { uploadPointImageBase64 } from './imageService';
...
/**
 * Créer un nouveau point avec une image optionnelle
 */
export const createPointWithImage = async (
  pointData: Omit<Point, 'id'>,
  imageFile?: File
): Promise<{ success: boolean; pointId?: string; imageId?: string; point?: any; error?: string }> => {
  try {
    // D'abord, créer le point
    const createPointResult = await createPoint(pointData);
    
    if (!createPointResult.success || !createPointResult.pointId) {
      throw new Error('Impossible de créer le point');
    }
    
    const pointId = createPointResult.pointId;
    console.log('📸 Point créé, traitement de l\'image...');
    
    // Si une image est fournie, l'uploader en base64
    let imageId: string | undefined;
    if (imageFile) {
      const uploadResult = await uploadPointImageBase64(imageFile, pointId);
      if (!uploadResult.success) {
        console.warn('⚠️ Attention: Point créé mais erreur lors de l\'upload d\'image:', uploadResult.error);
      } else {
        imageId = uploadResult.imageId;
        console.log('✅ Image base64 enregistrée avec succès');
      }
    }
    ...
    return {
      ...
      pointId,
      imageId,
      point: createPointResult.point
    };
    
  } catch (error: any) {
    console.error('❌ Erreur création point avec image:', error);
    return { success: false, error: error.message };
  }
};
